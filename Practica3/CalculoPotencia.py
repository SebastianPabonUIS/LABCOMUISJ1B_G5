...
class CalculoPotencia(gr.hier_block2, Qt.QWidget):
    def __init__(self, l_vect=1024):
        ...
        # Blocks
        ##################################################
        self.qtnumber = qtgui.number_sink(
            gr.sizeof_float,
            0,
        ...
        self.qtnumber.set_update_time(0.10)
        self.qtnumber.set_title("Potencia Logaritimica  [dBm]")
        ...
        for i in range(1):
            self.qtnumber.set_min(i, -1)
            self.qtnumber.set_max(i, 1)
            self.qtnumber.set_color(i, colors[i][0], colors[i][1])
            if len(labels[i]) == 0:
                self.qtnumber.set_label(i, "Data {0}".format(i))
            else:
                self.qtnumber.set_label(i, labels[i])
            self.qtnumber.set_unit(i, units[i])
            self.qtnumber.set_factor(i, factor[i])

        self.qtnumber.enable_autoscale(True)
        self._qtnumber_win = sip.wrapinstance(self.qtnumber.qwidget(), Qt.QWidget)
        self.top_layout.addWidget(self._qtnumber_win)
        ...
        self.connect((self.blocks_nlog10_ff_0, 0), (self.qtnumber, 0))
